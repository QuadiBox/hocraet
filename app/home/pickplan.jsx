...
import { onSnapshotWithCondition, addDocument } from "../db/firestoreService";
...
const Pickplan = ({cartAdder}) => {
    ...
    const handleAddToCart = (e) => {
        ...
        if (cartObject.plan === "diy") {
            ...
            const todb = {...cartObject, date: Date.now(), amount: totalAmount, mail: user?.emailAddresses[user?.emailAddresses.length - 1].emailAddress};

            addDocument("carts", todb);
        } else {
            const todb = {...cartObject, date: Date.now(), mail: user?.emailAddresses[user?.emailAddresses.length - 1].emailAddress};
            addDocument("carts", todb);
        }
    ...
    return (
        <section id="pricing" className="pricingSect">
            ...
            <div className="pickerCntn">
                <button onClick={() => {setCartObject(prev => ({...prev, plan: "basic", amount: 50, planData: pricingDetails[prev.platform].basic}))}} type="button" className={`pickBtn ${cartObject.plan !== "diy" && "active"}`}>Customized</button>
...

            {
                cartObject.plan !== "diy" && (
                    <div className="selectionDetail">
                        ...
                        <div className={`unitdetail ${selectPlan === "basic" && "active"}`}>
                            <h3>Basic</h3>
                        ...
                        <div className={`unitdetail ${selectPlan === "business" && "active"}`}>
                            <h3>Business</h3>
                        ...
                        <div className={`unitdetail ${selectPlan === "enterprise" && "active"}`}>
                            <h3>Enterprise</h3>
