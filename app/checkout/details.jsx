...
const Details = () => {
  ...
  const onClose = () => {
    // Implementation for whatever you want to do with reference and after success call.
    console.log("Payment portal closed");
  }

  const config = {
    reference: new Date().getTime().toString(),
    email: `${user?.emailAddresses[user?.emailAddresses.length - 1].emailAddress}`,
    amount: (totalAmount + (totalAmount * 0.02)) * 100 * 1650,
  ...
  useEffect(() => {
    const fetchCartItems = onSnapshotWithCondition(
      ...
      (documents) => {
        futureDate = new Date(Date.now() + twoWeeksInMilliseconds)
  ...
  return (
    <div className="confirmGrandCntn special">
      <div className="confirmCntn">
        ...
        {/* <button onClick={() => {initializePayment(onSuccess, onclose)}}>Checkout</button> */}
